...
static esp_err_t enc28j60_init(esp_eth_phy_t *phy)
{
    ...
    PHY_CHECK(eth->phy_reg_read(eth, enc28j60->addr, ETH_PHY_IDR2_REG_ADDR, &(id2.val)) == ESP_OK,
              "read ID2 failed", err);
    if (id1.oui_msb != 0x0083 || id2.oui_lsb != 0x05 || id2.vendor_model != 0x00) {
        ESP_LOGW(TAG, "Wrong chip ID: oui_msb=0x%x, oui_lsb=0x%x, vendor_model=0x%x", id1.oui_msb, id2.oui_lsb, id2.vendor_model);
        // Continue anyway
    }

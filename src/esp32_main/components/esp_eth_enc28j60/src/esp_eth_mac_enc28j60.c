...
static void emac_enc28j60_task(void *arg)
{
    ...
    while (1) {
loop_start:
        // If interrupt pin is available, block until notification or check the GPIO
        if (emac->int_gpio_num >= 0) {
            ...
            // Polling mode: wait a short time and then check packet counter
            if (ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(200)) == 0) {
                uint8_t pk_counter = 0;
                if (enc28j60_register_read(emac, ENC28J60_EPKTCNT, &pk_counter) == ESP_OK) {
                    if (pk_counter == 0) {
                        continue; // nothing to do
                    }
                    // emulate packet interrupt
                    status = EIR_PKTIF;
                } else {
                    continue;
                }
...
static esp_err_t emac_enc28j60_init(esp_eth_mac_t *mac)
{
    ...
    /* init gpio used for reporting enc28j60 interrupt if provided */
    if (emac->int_gpio_num >= 0) {
        ...
        ESP_LOGW(TAG, "INT GPIO not configured, using polling mode for ENC28J60");
    }
...
esp_eth_mac_t *esp_eth_mac_new_enc28j60(const eth_enc28j60_config_t *enc28j60_config, const eth_mac_config_t *mac_config)
{
    ...
    MAC_CHECK(emac, "calloc emac failed", err, NULL);
     /* enc28j60 driver prefers interrupt driven operation but can operate in polling mode
         if int_gpio_num < 0 (no interrupt pin connected). */
    /* SPI device init */
